...
namespace Clips
{
	...
	std::string FactToString(struct ::fact* fPtr)
	{
		...
		return "(" + std::string(fPtr->whichDeftemplate->header.name->contents) + " (" + str.substr(1) + "))";
	...
	std::vector<std::string> Environment::facts() const
	{
		...
		if (GetType(factlist) == MULTIFIELD)
		{
			...
			for (auto i = GetDOBegin(factlist); i <= GetDOEnd(factlist); i++)
			{
	...
	{
		EnvWatch(env, const_cast<char*>(str.c_str()));
	}
