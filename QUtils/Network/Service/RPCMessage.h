...
namespace Network
{
	...
	template <class Service_t, class Ret, class ...Args>
	class RPCMessage<const Service_t, Ret, Args...> : public ReturnMessage<Ret>
	{
		private:
		typedef Ret(Service_t::*Procedure_c)(Args...) const;
		typedef std::string Index;
		int _priority;
		...
		virtual void visit(Service& baseSrv) override
		{
			try
			{
			Procedure_c proc = ProcedureLookup<Index, Service_t, Ret, Args...>::Find_c(procIndex);
			Service_t& srv = static_cast<Service_t&>(baseSrv);
